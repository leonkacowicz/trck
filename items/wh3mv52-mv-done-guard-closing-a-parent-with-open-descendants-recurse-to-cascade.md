...
### Reconciled with #ge5jt9s (status rollup)

#ge5jt9s derives a parent's status *up* from its children. This issue is the *downward*
bulk-close convenience and is **fully compatible** — built on top of #ge5jt9s, it gets
simpler:
...
- **`--recurse` targets leaves only.** It moves the non-terminal *leaf* descendants to
  ...
  itself are then derived to terminal by #ge5jt9s's `normalize_statuses`. The target ends
...
- **The guard becomes a warning, not a block.** A plain `mv X <terminal>` (no
  `--recurse`) on a node with open descendants is #ge5jt9s's manual override: it pins `X`
...
- **Supersedes two earlier notes above:** the "independent of the points rollup" framing
  and the "no cascade-reopen (deliberate omission)" asymmetry no longer hold — #ge5jt9s
  provides upward derivation including reopen. `--recurse` remains downward-only and
  one-shot; reopen is #ge5jt9s's job, not this flag's.
- **Order:** depends on #ge5jt9s (this design assumes rollup is in place).
