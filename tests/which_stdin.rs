...
///
/// `Path` rather than a hand-rolled split, because the separator is the platform's business
/// and std already knows it: on Windows both `\` and `/` end a component (the listed paths
/// arrive as `\\?\D:\...\items\<id>-<slug>.md`, but MSYS tools hand out forward slashes), while
/// on Unix a backslash is an ordinary filename character. A `['/', '\\']` split would be wrong
/// there and would still miss the forward-slash case here.
///
/// The cost is that the Windows shape can only be exercised on Windows — which is what the
/// matrix job is for. The two tests below assert against paths the binary just printed, so they
/// are right on whichever platform they run.
