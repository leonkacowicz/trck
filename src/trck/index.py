...
def check_unique_ids(rows: list[Issue], origin: str = "index.jsonl") -> None:
    ...
    the in-memory model ambiguous, so it fails here — loudly, no guessing — like the
    malformed-JSON and wrong-type cases above, rather than as a `validate` error the
    mutating verbs would write straight past. (Downstream, `Graph.by_id` keeps one row
    while both are counted and listed, and `resolve_ref` returns `exact[0]`, so a verb
    would act on an arbitrary one of them.)

    Every duplicated id is collected before dying — one id per line, whatever the
    number of extra rows — so a single run reports the whole problem. The statuses are
    named because that is the detail identifying which side of a bad merge to keep;
    this is exactly the state `merge=union` on index.jsonl produces when two branches
    both mutate one issue."""
    seen, dupes = {}, {}
    for r in rows:
        if r.id in seen:
            dupes.setdefault(r.id, [seen[r.id]]).append(r)
        else:
            seen[r.id] = r
    if not dupes:
        return
    lines = [f"#{iid} appears {len(rs)} times (statuses: "
