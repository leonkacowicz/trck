...
def build_parser() -> argparse.ArgumentParser:
    ...
    mv.add_argument("status", help="backlog, ongoing, in-review or done")
    ...
    st = sub.add_parser("start", help="alias: move to ongoing",
                        description="Alias: move an issue to ongoing.")
    st.add_argument("id", help="issue id")
    st.set_defaults(func=cmd_start)

    rv = sub.add_parser(
        "review", help="alias: move to in-review (and link a PR)",
        formatter_class=raw,
        description="Alias: move an issue to in-review and — given a URL — record "
                    "it as the issue's pull request, in one step. An issue there stays "
                    "out of ready/next (nothing to pick up), but still blocks whatever "
                    "depends on it until the PR lands.",
    ...
    dn = sub.add_parser("done", help="alias: move to done",
                        description="Alias: move an issue to done.")
    ...
    se.add_argument("--priority",
                    help="urgent, high, medium, low or lowest")
