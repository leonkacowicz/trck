...
def cmd_show(args) -> None:
    ...
    # Both output paths end in reading the body, so guard once, here. A row whose file has
    # gone missing is an inconsistency `check` reports; `show` must report it too rather
    # than surface it as an uncaught FileNotFoundError. Mirrors `move_issue`'s guard.
    body_path = issue_path(ctx, row)
    if not body_path.exists():
        die(f"file missing for #{row.id}: {body_path}")
...
def cmd_list(args) -> None:
    ...
    if getattr(args, "paths", False):
        if as_json:
            die("--paths and --json are different output modes; pick one")
    ...
    if getattr(args, "flat", False):
        ...
        if as_json:
            emit_json([r.to_dict() for r in rows])
            return
    ...
    if as_json:
        # The same walk `forest_layout` does, kept as structure instead of flattened
        # with connector prefixes: same `shown` gate, same sibling `key`, same cycle
        # guard. `context` carries what the human view shows by dimming — a row pulled
        # in as a non-matching ancestor so its matched descendant does not float free.
        def node(n, seen):
            kids = [] if n.id in seen else [
                node(c, seen | {n.id}) for c in g.children_of(n, key=key)
                if c.id in shown]
            return {**n.to_dict(), "context": n.id in dim, "children": kids}
        emit_json([node(r, set()) for r in sorted(roots, key=key)])
        return
