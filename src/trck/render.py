...
def _shorten_lanes(order: list, pairs) -> list:
    """Slide single nodes down or up the order to shorten the gutter, keeping the result
    a linear extension. Returns a new order; `pairs` are the drawn `(prerequisite,
    dependent)` edges.
    ...
    `│` — shrink it and lanes close sooner, which narrows the graph and shortens the
    bridges. Prerequisites-first alone does not care: it will happily emit a root whose
    only dependent is at the bottom, leaving its lane open the whole way down.

    Gathered per node instead of per edge the cost collapses to
    ...
    candidate's delta reads off a prefix sum in constant time rather than costing a walk
    over the edges; without that the search is unusable (80 nodes took five seconds,
    against two milliseconds here). 200 nodes settle in ~40ms and 400 in ~280ms, both far
    past any real component — this repo's largest is 17.
    ...
    order = list(order)
    n = len(order)
    weight = dict.fromkeys(order, 0)
    after: dict = {}                         # after[v]:  v's prerequisites
    before: dict = {}                        # before[v]: v's dependents
    for a, b in pairs:
        weight[b] += 1
        weight[a] -= 1
    ...
    while True:
        moved = False
        at = {v: k for k, v in enumerate(order)}
        pref = prefix()
        for i in range(n):
            u = order[i]
            ...
            lo = max((at[p] for p in after.get(u, ())), default=-1) + 1
            hi = min((at[d] for d in before.get(u, ())), default=n) - 1
            for j in range(lo, hi + 1):
                if j == i:
                    continue
                # u itself travels j - i rows; everything it steps over shifts one row
                # the other way, and the prefix sum totals that block's weight at once.
                delta = (weight[u] * (j - i) - (pref[j + 1] - pref[i + 1])) if j > i else \
                        (weight[u] * (j - i) + (pref[i] - pref[j]))
                if delta < 0:
...
def _graph_topo(g: Graph, ids, hier: bool = True,
    ...
    Locality is a local rule, though: it settles each step without seeing what the choice
    costs further down, so it strands a root whose only dependent is far below. The order
    it produces is therefore the *starting* order, handed to `_shorten_lanes` to finish."""
    idset = set(ids)
    if edges is None:
        edges = drawn_edges(g, idset, hier, reduce=False)
    ...
    pairs = [(d, i) for i in idset for d in requires[i]]
    return _shorten_lanes(order, pairs), dependents, kinds
