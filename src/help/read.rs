...
pub(super) const VERBS: &[VerbHelp] = &[
    ...
    VerbHelp {
        ...
        blurb: "Read issue file paths (as positional args, or one per line on stdin when none are given) and print the matching issues in `list` format. The reverse of `path`/`list --paths`, for piping the output of a tool that works in file paths back into issues. For body search reach for `list --contains` instead, which answers from a ref-backed tracker too. Non-issue paths are skipped.",
    ...
    VerbHelp {
        ...
        opts: &[
            ...
            (
                "--contains CONTAINS",
                "case-insensitive substring filter on the issue's markdown body — literal text, not a regex. Full-text search as a filter, so it composes with every other option here. The body opens with its title heading, so this finds what --match would and more; --match stays for when you mean the title alone.",
            ),
    ...
    VerbHelp {
        ...
        blurb: "Serve the page `trck html` writes, from a process that renders it per request — so it shows the tracker as it is now rather than as it was when a file was last regenerated. Binds 127.0.0.1 and nothing else: there is no flag that widens it, because a tracker is a repository's working notes and nothing here should be reachable from a network. GET / is the page; /app.css and /app.js answer from the copies compiled into this binary, never from a file on disk. Edits staged on the page can be applied from it: the Apply button posts them back and this process runs them through the same verbs the CLI does, one commit each, pushed as usual. The panel shows the exact command each edit will run, and a refusal comes back as the engine's own diagnostic with the tracker untouched. An open page also keeps up on its own: it holds one connection to the server, which says so whenever the tracker moves — a write from here, or a fast-forward the poll loop found — and the page re-renders in place, keeping the selection, the scroll position and anything staged but not yet applied. A page written by `trck html` has no process behind it, so it goes on offering the commands to paste instead. A ref-backed tracker is also kept current: a timer fetches the tracker branch and fast-forwards the local ref when it is behind, which is the one place trck fetches without being asked — every read verb leaves the network alone. A diverged pair is reported, never resolved; an unreachable remote is reported and the local ref is served anyway. Ctrl-C stops it and frees the port.",
        args: &[],
        opts: &[
